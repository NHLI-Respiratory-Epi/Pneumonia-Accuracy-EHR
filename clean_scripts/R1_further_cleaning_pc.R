...
dat <- read.dta13("workingCohort_pc/18_add_covariates.dta", nonint.factors = TRUE)
...
table(dat$hosp_7days, useNA = "ifany")

dat <- dat %>% mutate(across(starts_with("icd"), ~ ifelse(hosp_7days == 0, NA, .x)))
dat <- dat %>% mutate(across(c(spno, epikey, epistart, epiend, date_interval), ~ ifelse(hosp_7days == 0, NA, .x)))
dat$admidate[dat$hosp_7days == 0] <- NA

# okay - save it here for analysis. 

table(dat$blood_positive)
table(dat$chest_xray)
table(dat$sputum_abnormal)
dat$blood_positive <- recode_factor(dat$blood_positive, "no positive blood result" = "no", "blood positive" = "yes")
dat$chest_xray <- recode_factor(dat$chest_xray, "no chest x-ray" = "no", "chest x-ray" = "yes")
dat$sputum_abnormal <- recode_factor(dat$sputum_abnormal, " yes" = "yes")
dat$abx <- recode_factor(dat$abx, "no antibiotics precribed" = "no", "antibiotics prescribed" = "yes")

dat <- dat %>% mutate(across(c(abx, blood_sent:cxray_symp_abx_blood_sputum_pos2), ~as.character(.)))
dat <- dat %>% mutate(across(c(abx, blood_sent:cxray_symp_abx_blood_sputum_pos2), ~ifelse(.x == "yes", "1", ifelse(.x == "no", "0", .x))))
dat <- dat %>% mutate(across(c(abx, blood_sent:cxray_symp_abx_blood_sputum_pos2), ~as.numeric(.)))

dat$abx_all_dur[is.na(dat$abx_all_dur)] <- 0

summary(dat$abx_all_dur)

# create the groups for the table

dat$hosp_pneumo_combo <- "Not admitted to hospital within 7 days"
dat$hosp_pneumo_combo[dat$icd_pneumo_first == 1] <- "Admitted to hospital within 7 days with pneumonia diagnosis" 
dat$hosp_pneumo_combo[dat$icd_pneumo_first == 0] <- "Admitted to hospital within 7 days with diagnosis other than pneumonia" 
table(dat$hosp_pneumo_combo)

dat$hosp_pneumo_combo <- factor(dat$hosp_pneumo_combo, levels = c("Not admitted to hospital within 7 days",
                                                                  "Admitted to hospital within 7 days with diagnosis other than pneumonia",
                                                                  "Admitted to hospital within 7 days with pneumonia diagnosis"))
                                                                  

dat_analysis <- dat

saveRDS(dat_analysis, "cleanCohort/1_further_cleaning_cohort_for_analysis_pc.RDS")
...
saveRDS(dat, "cleanCohort/1_further_cleaning_cohort_pc.RDS")
