...
dat <- readRDS("cleanCohort/1_further_cleaning_cohort_sc.RDS")

colnames(dat)

attributes(dat)

dat %>% select(primary42, GP_hosp_code_42) %>% table()

test <- summary_factorlist(dat, dependent = "primary42", explanatory = c("bmi_group", "gold"),)
test
...
table(dat$primary42)
prop.table(table(dat$primary42))
...
doc_1 <- read_docx() %>%
  body_add_par("Table 1B", style = "heading 1") %>%
  body_add_table(table1, style = "table_template")

print(doc_1, target = "Outputs/Table1B_new_sc.docx")
...
dat <- dat %>% filter(pneumo_term_over20_again != "")

nrow(dat)

over20s <- data.frame(table(dat$pneumo_term_over20_again))

over20s <- over20s %>% filter(Freq <180)

dat$pneumo_term_over180 <- dat$pneumo_term_over20_again
dat$pneumo_term_over180[dat$pneumo_term_over180 %in% over20s$Var1] <- "Other term"

table(dat$pneumo_term_over180)

# Calculate frequency of the categorical variable
dat_freq <- table(dat$pneumo_term_over180)
...
dat_freq <- as.data.frame(table(dat$pneumo_term_over180))
...
ggplot(dat_freq, aes(x = category, y = frequency)) +
  ...
  theme_minimal() +
  labs(x = "Pneumonia term", y = "Frequency", title = "Frequency of pneumonia terms recorded in primary care after hospitalisation") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) # Rotate x-axis labels if needed

ggsave("Outputs/pneumonia_codes_breakdown_sc.jpeg")


table(dat$pneumo_term_over20_again)
